...
- **コード**: `agent/planner.py`、`agent/executor.py`（Epic C で実装予定・想定パス）
- **図案**: プランナー→エグゼキュータの協調動作を示すシーケンス図
...
- **コード**: `agent/reflection.py`（Epic B で実装予定・想定パス）
- **図案**: 抽出→自己批評→再確認/フォールバックのループ図
- **執筆タイミング**: Epic B 完了後
...
- **コード**: koto-log ADR本文（`docs/adr/` 想定パス）
- **図案**: 単一ReActとMulti-Agentのコスト・複雑性比較図
...
- **コード**: 深追いしない。B-0同様、図解と各論記事へのリンクが主役
- **図案**: 静的/動的・知識/接続の4要素マッピング図
