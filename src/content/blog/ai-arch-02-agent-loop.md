...
「今日は何回飲んだ？」の場合、①LLMが `query_records` を選ぶ → ②アプリがDBを集計する → ③**その結果をもう一度LLMに渡して**「今日は5回、合計600ml飲んだ」という文章にしてもらう、まで必要です。ツールの実行結果を見てLLMが次の行動（別のツールを呼ぶ・文章を返す・聞き返す）を決める——この反復がエージェントループです。
