...
LLMを組み込んだアプリを作り始めると、最初にぶつかるのが「どこまでLLMに任せるか」という線引きです。何でもできるように見えるので全部任せたくなるものの、それをやると数字の不一致・時刻のずれ・集計カテゴリの分裂といった「たまに間違うシステム」ができあがります。
...
[koto-log](https://github.com/Kaaaaazuya/koto-log) — LINEで動く育児記録エージェントです。「3時にミルク120ml飲んだ」と送れば構造化して保存されます。「今日何回飲んだ？」と聞けば集計して返してくれます。中核はLLMのTool Use：入力に応じてLLMが「どのツールを・どの引数で呼ぶか」を判断し、アプリ側のコードがDBを更新・参照します。
...
[全文はこちら](https://github.com/Kaaaaazuya/koto-log/blob/20845c04ee5716d323525ed765c61fc0bb1be34a/src/kotolog/utils/timeparse.py)。正規表現ベースの地味な実装ですけれど、**同じ入力には必ず同じ結果を返し、テストが書ける**点に価値があります。「3時」を「直近の過去の3時」に倒す仕様（未来の記録は存在しないため）のような業務ルールも、プロンプトではなくコードに置くことで確実に効きます。認識できない入力は安全側に倒して現在時刻を返します。
...
**小型モデルでも成立する**: LLMの仕事が「ツールを選んで引数を埋める」だけまで削られているので、ローカルの小型モデルでも実用になります。実際koto-logはローカルのOllamaで無料稼働します。モデル文字列の変更だけでClaudeにも切り替えられます（この抽象化はシリーズ第5回で扱います）。

**テストとevalsを分離できる**: 決定的な部分（timeparse・subtype・executor）はpytestで網羅し、非決定的な部分（ツール選択）はevalsで正答率を計測します。この評価は第7回で扱います。
